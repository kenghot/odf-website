...
interface IAccountRecievableReport
...
@inject("appStore")
@observer
class AccountRecievableReport extends React.Component<
  IAccountRecievableReport
> {
  public state = {
    fiscalYear: "",
    region: "",
  ...
  public render() {
    ...
    return (
      <ReportCard
        title={t("module.report.AccountRecievableReport.title")}
        filter={
  ...
  private onGetReport = async () => {
    ...
    try {
      await AccountReceivable.get(
        ...
        { name: "printAccountRecievableReport" }
      );
    } catch (e) {
...
export default withRouter(withTranslation()(AccountRecievableReport));

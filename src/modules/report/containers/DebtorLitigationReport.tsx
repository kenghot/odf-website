...
import { DebtCollection } from "../../debtCollection/DebtCollectionsService";
const reportUrl = `${process.env.REACT_APP_DEBT_ENDPOINT}`;

interface IDebtorLitigationReport extends WithTranslation, RouteComponentProps {
  appStore?: IAppModel;
...
@inject("appStore")
@observer
class DebtorLitigationReport extends React.Component<IDebtorLitigationReport> {
  ...
  public render() {
    ...
    return (
      <ReportCard
        title={t("module.report.DebtorLitigationReport.title")}
        filter={
  ...
  private onGetReport = async () => {
    ...
    try {
      await DebtCollection.get(
        ...
        { name: "printLitigationReport" }
      );
      // await fetchNoService(`${reportUrl}/report_01.php`, {
...
export default withRouter(withTranslation()(DebtorLitigationReport));

...
interface ITransactionLogReport
...
@inject("appStore")
@observer
class TransactionLogReport extends React.Component<
  ITransactionLogReport
> {
  public state = {
    paidDate: "",
    yearMonth: undefined,
  ...
  public render() {
    ...
    return (
      <ReportCard
        title={t("module.report.CounterServiceTransactionLogReport.title")}
        filter={
  ...
  private onGetReport = async () => {
    ...
    try {
      await CounterService.get(
        ...
        { name: "printCounterServiceTransactionLogReport" }
      );
    } catch (e) {
...
export default withRouter(
  withTranslation()(TransactionLogReport)
);

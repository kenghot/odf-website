...
interface ICancelPaymentReport extends WithTranslation, RouteComponentProps {
  appStore?: IAppModel;
...
@inject("appStore")
@observer
class CancelPaymentReport extends React.Component<ICancelPaymentReport> {
  ...
  public render() {
    ...
    return (
      <ReportCard
        title={t("module.report.CancelPaymentviaKTBReport.title")}
        filter={
  ...
  private onGetReport = async () => {
    ...
    try {
      await Report.get(
        ...
        { name: "printCancelPaymentviaKTBReport" }
      );
    } catch (e) {
...
export default withRouter(withTranslation()(CancelPaymentReport));

...
interface IDonationOlderFundSearchForm extends WithTranslation {
...
@inject("appStore")
@observer
class DonationDirectsSearchForm extends React.Component<IDonationOlderFundSearchForm> {
      ...
      <Grid columns={2}>
        <Grid.Row verticalAlign="top">
          <Grid.Column width={10}>
            <Header
              size="medium"
              content={t("module.donation.DonationDirectsSearchForm.header")}
              subheader={t(
  ...
  private renderHideHeader() {
    const { t, donationDirectListStore, appStore } = this.props;
    return (
      <Form.Group widths="equal">
        <Form.Input
          ...
          value={donationDirectListStore.filterDocumentNumber}
        />
        <Form.Input
          fluid
          label={t("module.donation.DonationDirectsSearchForm.filterOrgName")}
          placeholder={t(
            "module.donation.DonationDirectsSearchForm.filterOrgNamePlaceholder"
          )}
          onChange={(event, data) =>
            this.onChangeInputField("filterOrgName", data.value)
          }
          value={donationDirectListStore.filterOrgName}
        />
        <Form.Field
          label={t(
            "module.donation.DonationDirectsSearchForm.filterOrganizationId"
          )}
          placeholder={t(
            "module.donation.DonationDirectsSearchForm.filterOrganizationIdPlaceholder"
          )}
          control={OrganizationDDL}
          value={donationDirectListStore.filterOrganizationId}
    ...
    return (
      <React.Fragment>
        ...
        <Form.Field
          ...
          valueFieldNameIdCard={donationDirectListStore.filterSponsorIdCardNo}
          valueFieldNameFirstname={
            donationDirectListStore.filterSponsorFirstname
          }
          valueFieldNameLastname={donationDirectListStore.filterSponsorLastname}
          size="mini"
        />
        ...
        <Form.Field
          ...
          valueFieldNameStartDate={donationDirectListStore.filterStartDate}
          valueFieldNameEndDate={donationDirectListStore.filterEndDate}
          size="mini"
        />
      </React.Fragment>
    );
  ...
  private onSearch = async () => {
    ...
    await donationDirectListStore.setField({
      fieldname: "currentPage",
      value: 1,
    });
    await donationDirectListStore.load_data();
  ...
  private resetFilter = async () => {
    const { donationDirectListStore } = this.props;
    await donationDirectListStore.resetFilter();
    await donationDirectListStore.load_data();
  ...
  private onChangeInputField = (fieldname: string, value: any) => {
    const { donationDirectListStore } = this.props;
    donationDirectListStore.setField({ fieldname, value });
  };
}
...
export default withTranslation()(DonationDirectsSearchForm);

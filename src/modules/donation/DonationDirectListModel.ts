...
import {
  DonationDirectModel,
  IDonationDirectModel,
} from "./DonationDirectModel";
import {
  DonationDirects,
  DonationDoc,
  DonationDocUrl,
...
export const DonationDirectListModel = types
  .model("DonationDirectListModel", {
    list: types.optional(types.array(DonationDirectModel), []),
    ...
    donationFile: types.maybe(customtypes.FilePrimitive),
  ...
  .views((self: any) => ({
    get statusMenu() {
      return self.list.find(
        (item: IDonationDirectModel) => item.isSelected === true
      )
        ? false
        : true;
    },
    get selected_checkbox() {
      if (self.list.length > 0) {
        const selectedList = self.list.map(
          (item: IDonationDirectModel) => item.isSelected
        );
    ...
    get donation_file() {
      if (self.donationFile) {
        return [self.donationFile];
      } else {
        return [];
      }
    },
    get donation_list_id_check() {
      return self.list
        .filter((item: IDonationDirectModel) => item.isSelected === true)
        .map((item: IDonationDirectModel) => item.id);
    },
  ...
  .actions((self: any) => ({
    ...
    selected_all: (selected: boolean) => {
      ...
      self.list.forEach((item: IDonationDirectModel) => {
    ...
    load_data: flow(function* () {
      try {
        ...
        // const result: any = yield require("./donationDirectsList.json");
        const result: any = yield DonationDirects.get(body);
    ...
    resetFilter: flow(function* () {
      try {
        ...
        const result: any = yield DonationDirects.get({
          perPage: self.perPage,
        });
        self.setField({ fieldname: "list", value: result.data });
        self.setPagination(result.currentPage, result.totalPages, result.total);
    ...
    printThankyouLetters: flow(function* (
      ...
      try {
        ...
        const result: any = yield fetchNoService(
          `${DonationDocUrl}/opentbs/tbs/template_thankyou_direct.php`,
          {
            show_debug: "0",
    ...
    printEnvelops: flow(function* (
      ...
      try {
        self.setField({ fieldname: "loading", value: true });
        const result: any = yield fetchNoService(
          `${DonationDocUrl}/opentbs/tbs/template_envelop_direct.php`,
          {
            show_debug: "0",
            ids: self.donation_list_id_check.join(","),
            isReport,
    ...
    generateEDonation: flow(function* () {
      try {
        self.setField({ fieldname: "loading", value: true });
        yield DonationDirects.create(
          {
            ids: self.donation_list_id_check,
          },
          { name: "generate_e_donation" }
        );
        self.error.setField({ fieldname: "tigger", value: false });
        self.alert.setAlertMessage("", "");
      } catch (e) {
...
export type IDonationDirectListModel = typeof DonationDirectListModel.Type;
